...
top_month, top_rev = max(revenue_by_month.items(), key=lambda x: x[1])
print(f"Month with Highest Revenue: {top_month} (${top_rev:.2f})")
...
top_product = max(avg_order, key=lambda p: avg_order[p])
print(f"Product with Highest Average Order Value: {top_product} (${avg_order[top_product]:.2f})")
